...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                "Brendon Hartley (BRT) hasn`t enough data. Not included in results",
                "FAM 2018-05-24_12:13:04.512 not included in results -- wrong data format in line 5 of resource file.",
            ],
        ),
    ],
)
def test_get_drivers_list(drivers_info, start_data, end_data, drivers_data, error_list):
    ...
    assert code.get_drivers_list(drivers_info, start_data, end_data) == (
        drivers_list,
        error_list,
    )
