...
def get_datetime_info(time_info: list, errors: list) -> tuple: # dict('driver abbreviation' : driver time os start/finish in datetime format, ...)
    datetime_dict = {}
    if type(time_info) != list:
        ...
        for item in enumerate(time_info):
            if item[1] in ('', ' ', '_', None):
                ...
                errors.append(f'{item[1]} not included in results -- wrong data format in line {item[0] + 1} of resource file.')
            else:
...
def get_drivers_list(drivers_info: list, start_data: list, end_data: list) -> tuple:
    ...
    for item in enumerate(drivers_info):
        if item[1] in ('', ' ', '_', None):
            ...
            error_list.append(f'{item[1]} not included in results -- wrong data format in line {item[0] + 1} of resource file.')
        else:
            ...
            try:
                ...
                error_list.append(f'{driver_data[1]} ({driver_data[0]}) hasn`t enough data. Not included in results')
...
def get_data() -> tuple:
    with (open(f'{BASE_DIR}/log_data/abbreviations.txt') as drivers,
        ...
        return drivers.read().splitlines(), start.read().splitlines(), end.read().splitlines()
...
def get_list_info(order: str, attr: str) -> tuple:
    ...
    elif order not in ('asc', 'desc', None):
        raise ValueError('Wrong value of sorted attribute')
    else:
        data = get_data()
        drivers_list = get_drivers_list(*data)
        result = list(sorted(drivers_list[0], key=lambda x: getattr(x, attr)))
...
def get_driver_info(driver_id: str):
    ...
    return list(filter(lambda el: el.abbr == driver_id, results))[0]
